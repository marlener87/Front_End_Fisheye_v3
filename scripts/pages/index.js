  ...
  objectPhotographers.photographers.forEach((item) => {
    ...
    divCard.innerHTML = `
        <a href="photographer.html?id=${photographer.id}" class="lienPhotographe" tabindex="${tabindexCounter++}">
          <img src="../assets/photographersIDPhotos/${photographer.portrait}" alt="image de ${photographer.name}" class="portrait">
          <h2 class="name" aria-label="nom du photographe">${photographer.name}</h2>
        </a>
        <p class="localisation" aria-label="localisation du photographe : ${photographer.city}, ${photographer.country}" tabindex="${tabindexCounter++}">${photographer.city}, ${photographer.country}</p>
        <p class="tagline" aria-label="citation du photographe : ${photographer.tagline}" tabindex="${tabindexCounter++}">${photographer.tagline}</p>
        <p class="price" aria-label="tarif à la journée du photographe : ${photographer.price}€ par jour" tabindex="${tabindexCounter++}">${photographer.price}€/jour</p>
    `;
